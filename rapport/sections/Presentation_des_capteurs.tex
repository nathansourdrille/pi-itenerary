...
\subsection{Démarche}
La calibration vise à réduire les erreurs systématiques. Pour l’accéléromètre et le gyroscope, nous avons mesuré les valeurs à l’arrêt pour corriger les biais. Pour le magnétomètre, une procédure de calibration en huit (figure en 8) a été utilisée pour ajuster les axes.
...
\begin{itemize}
    \item \textbf{Filtrage passe-bas} sur les données d'accéléromètre et gyroscope pour réduire le bruit haute fréquence.
    \item \textbf{Filtre de Kalman} (ou complémentaire) pour fusionner les données de plusieurs capteurs (IMU).
    \item \textbf{Moyennes glissantes} pour lisser les données GPS.
\end{itemize}
