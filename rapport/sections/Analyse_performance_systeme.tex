...
Dans les cas où les capteurs proposaient un grand nombre de configurations possibles (notamment en termes de plages et de fréquences d’échantillonnage), nous avons choisi d’étalonner un sous-ensemble représentatif des réglages. L’objectif était de couvrir un éventail suffisamment large pour observer les effets des paramètres sans multiplier de manière excessive les combinaisons.
...
\subsubsection{Test d'homogénéité des variances}
...
\subsubsection{Sélection empirique du meilleur paramétrage}
...
aux données du GPS seulement. C'est l'une des raisons pour lequelles on doit appliquer un filtre de Kalman et fusionner les capteurs.
...
Les graphes ci-dessus montrent clairement que les données du GPS, prises seules, sont peu exploitables en raison du grand nombre de valeurs aberrantes, notamment lorsque le système est immobile. Cette instabilité s’explique par la faible précision instantanée du GPS grand public, soumise à de nombreux facteurs comme expliqué dans la section ~\ref{sec : sources de bruitages}. Pour améliorer la qualité des mesures, il est essentiel d’attendre un certain seuil de temps — quelques secondes — après la mise en route afin de permettre aux capteurs de se stabiliser et de se calibrer correctement.
